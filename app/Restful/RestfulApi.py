...
from exts import db
import glob, time, os, stat, sys, ast
from configs import KEY_TOKEN
import paramiko,re,subprocess,threading,time
...
class ScriptGenerationView(Resource):
    ...
    def post(self):
        ...
        def type_post():
            # 本机测试脚本路径
            file = glob.glob('/data/soft/Quality_Platform/scripts/qa-post-test.py')
            # 定义自动生成脚本函数
            for one_polling in file:
                ...
                new_file = "/data/soft/Quality_Platform/scripts/{username}-qa-test-{date}.py".format(date=date,username=username)
        ...
        def type_get():
            # 本机测试脚本路径
            file = glob.glob('/data/soft/Quality_Platform/scripts/qa-get-test.py')
            # 定义自动生成脚本函数
            for one_polling in file:
                username = session.get('username')
                username = str(username)
                username = username.split('@')[0] #去除邮箱后缀
                date = time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime())
                new_file = "/data/soft/Quality_Platform/scripts/{username}-qa-test-{date}.py".format(date=date,
                                                                                               username=username)
                filename = '{username}-qa-test-{date}.py'.format(date=date, username=username)
                put_file = "/data/Qa_Quality/scripts/" + "{username}-qa-test-{date}.py".format(date=date,
                                                                                               username=username)
...
class ScheduledTasksView(Resource):
    def post(self):
        ...
        scheduled_log = open('/scheduled_{username}.log'.format(username=uu),mode='w+',encoding='utf-8')
        ...
        def ssh2(ip, username, password, cmd):
            try:
                ...
                for i in lines: #将输出结果存入日志文件
                    print(i,file=scheduled_log)
                scheduled_log.close()
