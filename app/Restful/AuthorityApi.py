...
class AuthUser(Resource):   # 用户的增删改查
    ...
    def post(self):
        ...
        role = req_dict.get("role")    # 用户角色 0为开发，1为测试
        ...
        try:
            db.session.add(
                db_mysql.User(username=username, password=generate_password_hash(password),
                              name=name, role=role, ip_type=ip_type))
            db.session.commit()
...
class AuthGroupFunc(Resource):  # 组权限的增删改查
    ...
    def get(self):
        # 获取url请求的数据
        tree_id = request.args.get("tree_id")  # 项目id
        project_id = request.args.get("project_id")   # 项目工程id
